...
export async function GET(req: NextRequest) {
  ...
  const monthly = Object.entries(monthMap).sort(([a], [b]) => b.localeCompare(a)).map(([month, v]) => ({ month, ...v }))
  ...
  if (exportCsv) {
    ...
    const rows = filtered.map((g: any) => [
      ...
      g.completion_tokens, g.total_tokens, g.cost_usd, g.model, g.match_id,
    ])
    ...
    return new NextResponse(csv, { headers: { 'Content-Type': 'text/csv', 'Content-Disposition': `attachment; filename="costs.csv"` } })
  }
